...
Set `PORT` in `.env` to change the development port:

```dotenv
PORT=4000
```

The production server started with `npm start` uses the same setting.
...
The application is available at `http://localhost:${PORT}`. Stop it with:
...
```bash
docker build -t rmkasendwa .
docker run --rm -p 3000:3000 -e PORT=3000 rmkasendwa
```
